...
void VGA::IOReadStatusRegister1(u8* value)
{
...
void VGA::IOCRTCDataRegisterRead(u8* value)
{
...
void VGA::IOCRTCDataRegisterWrite(u8 value)
{
  if (m_crtc_index_register >= countof(m_crtc_registers.index))
  {
...
void VGA::IOGraphicsDataRegisterRead(u8* value)
{
...
void VGA::IOGraphicsDataRegisterWrite(u8 value)
{
...
void VGA::IOMiscOutputRegisterWrite(u8 value)
{
...
void VGA::IOAttributeAddressRead(u8* value)
{
  *value = m_attribute_address_register;
}

void VGA::IOAttributeDataRead(u8* value)
{
...
void VGA::IOAttributeAddressDataWrite(u8 value)
{
...
void VGA::IOSequencerDataRegisterRead(u8* value)
{
...
void VGA::IOSequencerDataRegisterWrite(u8 value)
{
...
void VGA::IODACReadAddressWrite(u8 value)
{
...
void VGA::IODACWriteAddressWrite(u8 value)
{
...
void VGA::IODACDataRegisterRead(u8* value)
{
...
void VGA::IODACDataRegisterWrite(u8 value)
{
...
bool VGA::MapToVRAMOffset(u32* offset)
{
  u32 value = *offset;
...
void VGA::HandleVRAMRead(u32 offset, u8* value)
{
  ...
  if (m_sequencer_registers.sequencer_memory_mode.chain_4_enable)
  {
    ...
    latch_linear_address = ((offset & ~u32(3)) << 2) & (VRAM_SIZE - 1);
  ...
  u32 read_linear_address = (offset << 2) & (VRAM_SIZE - 1);
  if (m_graphics_registers.mode.host_odd_even)
  {
    ...
    read_linear_address = (offset & ~u32(1)) * 4;
  }
...
void VGA::HandleVRAMWrite(u32 offset, u8 value)
{
  if (!MapToVRAMOffset(&offset))
  {
    // Out-of-range for the current mapping mode
  ...
  if (m_sequencer_registers.sequencer_memory_mode.chain_4_enable)
  {
    ...
    if (m_sequencer_registers.memory_plane_write_enable & (1 << plane))
    {
      ...
      //      6 |     2 |                 4 |           18
      //      7 |     3 |                 4 |           19
      offset = ((offset & ~u32(3)) * 4) | ZeroExtend32(plane);
  ...
  else if (!m_sequencer_registers.sequencer_memory_mode.odd_even_host_memory)
  {
    ...
    if (m_sequencer_registers.memory_plane_write_enable & (1 << plane))
    {
      ...
      offset = ((offset & ~u32(1)) * 4) | ZeroExtend32(plane);
      DebugAssert(offset < VRAM_SIZE);
      m_vram[offset] = value;
    }
  }
...
void VGA::RecalculateEventTiming()
{
  ...
  u32 pixel_clock = pixel_clocks[m_misc_output_register.clock_select];

  // Character width depending on dot clock - is this correct for graphics modes? What about dot clock rate?
  u32 character_width = m_sequencer_registers.clocking_mode.dot_mode ? 8 : 9;
  ...
  u32 horizontal_total_pixels = (ZeroExtend32(m_crtc_registers.horizontal_total) + 5) * character_width;
  ...
  u32 vertical_active_lines = (ZeroExtend32(m_crtc_registers.vertical_display_end) |
...
u32 VGA::CRTCReadVRAMPlanes(u32 address_counter, u32 row_scan_counter) const
{
  u32 address = CRTCWrapAddress(address_counter, row_scan_counter);
  u32 vram_offset = address * 4;
  u32 all_planes;
...
u32 VGA::CRTCWrapAddress(u32 address_counter, u32 row_scan_counter) const
{
  u32 address;
...
{
  u32 character_height = (m_crtc_registers.maximum_scan_lines & 0x1F) + 1;
  u32 character_width = 8;
  if (!m_sequencer_registers.clocking_mode.dot_mode)
    character_width = 9;
  if (m_sequencer_registers.clocking_mode.dot_clock_rate)
    character_width = 16;

  u32 character_columns = (u32(m_crtc_registers.end_horizontal_display) + 1);
  u32 character_rows = (m_crtc_registers.vertical_display_end | (u32(m_crtc_registers.overflow_register & 0x02) << 7) |
                        (u32(m_crtc_registers.overflow_register & 0x40) << 3));
...
void VGA::DrawTextGlyph8(u32 fb_x, u32 fb_y, const u8* glyph, u32 rows, u32 fg_color, u32 bg_color, s32 dup9)
{
...
void VGA::DrawTextGlyph16(u32 fb_x, u32 fb_y, const u8* glyph, u32 rows, u32 fg_color, u32 bg_color)
{
  const u32 colors[2] = {bg_color, fg_color};
...
{
  u32 screen_width = (u32(m_crtc_registers.end_horizontal_display) + 1) * 8;
  u32 screen_height = (m_crtc_registers.vertical_display_end | (u32(m_crtc_registers.overflow_register & 0x02) << 7) |
                       (u32(m_crtc_registers.overflow_register & 0x40) << 3)) +
                      1;
  if (screen_width == 0 || screen_height == 0)
    return;
  ...
  u32 scanlines_per_row = ZeroExtend32(m_crtc_registers.maximum_scan_lines & 0x1F) + 1;
    ...
    if (!m_graphics_registers.mode.shift_256)
    {
      ...
      else
      {
        ...
        for (s32 col = -(s32)horizontal_pan; col < (s32)screen_width;)
        {
          ...
          for (u32 subcol = 0, subindex = 0; col < (s32)screen_width && subindex < 8;)
          {
...
#else
      s32 col = -s32(pan_pixels);
      s32 screen_width_div2 = s32(screen_width / 2);
#endif
      while (col < 0)
      {
