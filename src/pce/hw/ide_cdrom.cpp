...
PROPERTY_TABLE_MEMBER_UINT("Device", 0, offsetof(IDECDROM, m_ide_device), nullptr, 0)
END_OBJECT_PROPERTY_MAP()

IDECDROM::IDECDROM(const String& identifier, u32 ide_channel /* = 0 */, u32 ide_device /* = 0 */,
                   const ObjectTypeInfo* type_info /* = &s_type_info */)
  : m_ide_channel(ide_channel), m_ide_device(ide_device), BaseClass(identifier, type_info)
{
}
...
bool IDECDROM::Initialize(System* system, Bus* bus)
{
  ...
  if (!hdc->AttachATAPIDevice(m_ide_device, this))
  {
    Log_ErrorPrintf("Failed to attach CDROM to IDE channel %u device %u", m_ide_channel, m_ide_device);
    return false;
  }
