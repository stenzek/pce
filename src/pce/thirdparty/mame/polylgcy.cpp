...
struct work_unit_shared
{
  ...
  std::atomic<u32> count_next; /* number of scanlines and index of next item to process */
  int16_t scanline;            /* starting scanline and count */
  uint16_t previtem;           /* index of previous item in the same bucket */
#ifndef PTR64
  u32 dummy; /* pad to 16 bytes */
#endif
};
...
struct polygon_info
{
  ...
  u8 numparams;                                 /* number of parameters for this polygon  */
  u8 numverts;                                  /* number of vertices in this polygon */
  poly_draw_scanline_func callback;             /* callback to handle a scanline's worth of work */
  s32 xorigin;                                  /* X origin for all parameters */
  s32 yorigin;                                  /* Y origin for all parameters */
...
struct legacy_poly_manager
{
  ...
  u32 unit_next;    /* index of next unit to allocate */
  u32 unit_count;   /* number of work units available */
  size_t unit_size; /* size of each work unit, in bytes */

  /* quad work units */
  u32 quadunit_next;    /* index of next unit to allocate */
  u32 quadunit_count;   /* number of work units available */
  size_t quadunit_size; /* size of each work unit, in bytes */

  /* poly data */
  polygon_info** polygon; /* array of polygon pointers */
  u32 polygon_next;       /* index of next polygon to allocate */
  u32 polygon_count;      /* number of polygon items available */
  size_t polygon_size;    /* size of each polygon, in bytes */

  /* extra data */
  void** extra;      /* array of extra data pointers */
  u32 extra_next;    /* index of next extra data to allocate */
  u32 extra_count;   /* number of extra data items available */
  ...
  /* statistics */
  u32 triangles; /* number of triangles queued */
  u32 quads;     /* number of quads queued */
  u64 pixels;    /* number of pixels rendered */
#if KEEP_STATISTICS
  u32 unit_waits;                  /* number of times we waited for a unit */
  u32 unit_max;                    /* maximum units used */
  u32 polygon_waits;               /* number of times we waited for a polygon */
  u32 polygon_max;                 /* maximum polygons used */
  u32 extra_waits;                 /* number of times we waited for an extra data */
  u32 extra_max;                   /* maximum extra data used */
  u32 conflicts[WORK_MAX_THREADS]; /* number of conflicts found, per thread */
  u32 resolved[WORK_MAX_THREADS];  /* number of conflicts resolved, per thread */
#endif
};
...
static void** allocate_array(size_t* itemsize, u32 itemcount);
...
static inline void convert_tri_extent_to_poly_extent(poly_extent* dstextent, const tri_extent* srcextent,
                                                     const polygon_info* polygon, s32 y)
{
  ...
  {
    dstextent->param[paramnum].start = polygon->param[paramnum].start +
                                       srcextent->startx * polygon->param[paramnum].dpdx +
                                       y * polygon->param[paramnum].dpdy;
...
static inline void interpolate_vertex(poly_vertex* outv, const poly_vertex* v1, const poly_vertex* v2, int paramcount,
                                      float clipval)
{
...
legacy_poly_manager* poly_alloc(int max_polys, size_t extra_data_size, u8 flags)
{
  /* allocate the manager itself */
  legacy_poly_manager* poly = new legacy_poly_manager();
  std::memset(poly, 0, sizeof(legacy_poly_manager));
  poly->flags = flags;
...
void poly_free(legacy_poly_manager* poly)
{
#if KEEP_STATISTICS
  {
    ...
    if (poly->pixels > 1000000000)
      printf("Total pixels   = %d%09d\n", (u32)(poly->pixels / 1000000000), (u32)(poly->pixels % 1000000000));
    else
      printf("Total pixels   = %d\n", (u32)poly->pixels);
    printf("Conflicts:  %d resolved, %d total\n", resolved, conflicts);
    printf("Units:      %5d used, %5d allocated, %5d waits, %4d bytes each, %7d total\n", poly->unit_max,
           poly->unit_count, poly->unit_waits, poly->unit_size, poly->unit_count * poly->unit_size);
    printf("Polygons:   %5d used, %5d allocated, %5d waits, %4d bytes each, %7d total\n", poly->polygon_max,
           poly->polygon_count, poly->polygon_waits, poly->polygon_size, poly->polygon_count * poly->polygon_size);
    printf("Extra data: %5d used, %5d allocated, %5d waits, %4d bytes each, %7d total\n", poly->extra_max,
           poly->extra_count, poly->extra_waits, poly->extra_size, poly->extra_count * poly->extra_size);
  }
#endif
...
u32 poly_render_triangle(legacy_poly_manager* poly, void* dest, const rectangle& cliprect,
                         poly_draw_scanline_func callback, int paramcount, const poly_vertex* v1, const poly_vertex* v2,
                         const poly_vertex* v3)
{
  ...
  {
    u32 bucketnum = ((u32)curscan / SCANLINES_PER_BUCKET) % TOTAL_BUCKETS;
    u32 unit_index = poly->unit_next++;
    tri_work_unit* unit = &poly->unit[unit_index]->tri;
    int extnum;

    /* determine how much to advance to hit the next bucket */
    scaninc = SCANLINES_PER_BUCKET - (u32)curscan % SCANLINES_PER_BUCKET;
  ...
  if (poly->queue != nullptr)
  {
    osd_work_item_queue_multiple(poly->queue, poly_item_callback, poly->unit_next - startunit, poly->unit[startunit],
                                 poly->unit_size, WORK_ITEM_FLAG_AUTO_RELEASE);
  }
...
u32 poly_render_triangle_fan(legacy_poly_manager* poly, void* dest, const rectangle& cliprect,
...
u32 poly_render_triangle_custom(legacy_poly_manager* poly, void* dest, const rectangle& cliprect,
...
u32 poly_render_quad(legacy_poly_manager* poly, void* dest, const rectangle& cliprect, poly_draw_scanline_func callback,
                     ...
                     const poly_vertex* v4)
{
  ...
  for (curscan = minyclip; curscan < maxyclip; curscan += scaninc)
  {
    ...
    for (extnum = 0; extnum < unit->shared.count_next; extnum++)
    {
      ...
      {
        for (paramnum = 0; paramnum < paramcount; paramnum++)
          unit->extent[extnum].param[paramnum].start +=
            (cliprect.min_x - istartx) * unit->extent[extnum].param[paramnum].dpdx;
        istartx = cliprect.min_x;
      }
      if (istopx > cliprect.max_x)
        istopx = cliprect.max_x + 1;
...
u32 poly_render_quad_fan(legacy_poly_manager* poly, void* dest, const rectangle& cliprect,
                         poly_draw_scanline_func callback, int paramcount, int numverts, const poly_vertex* v)
{
  u32 pixels = 0;
  int vertnum;
  ...
  for (vertnum = 2; vertnum < numverts; vertnum += 2)
    pixels += poly_render_quad(poly, dest, cliprect, callback, paramcount, &v[0], &v[vertnum - 1], &v[vertnum],
                               &v[std::min(vertnum + 1, numverts - 1)]);
  return pixels;
}
...
u32 poly_render_polygon(legacy_poly_manager* poly, void* dest, const rectangle& cliprect,
                        poly_draw_scanline_func callback, int paramcount, int numverts, const poly_vertex* v)
{
...
static void** allocate_array(size_t* itemsize, u32 itemcount)
{
  ...
  *itemsize = ((*itemsize + CACHE_LINE_SIZE - 1) / CACHE_LINE_SIZE) * CACHE_LINE_SIZE;
  ...
  /* allocate the array */
  ptrarray = static_cast<void**>(std::calloc(itemcount, sizeof(void*)));

  /* allocate the actual items */
  ptrarray[0] = std::calloc(*itemsize * itemcount, sizeof(u8));

  /* initialize the pointer array */
  for (itemnum = 1; itemnum < itemcount; itemnum++)
    ptrarray[itemnum] = (u8*)ptrarray[0] + *itemsize * itemnum;
  return ptrarray;
}
...
static void* poly_item_callback(void* param, int threadid)
{
  while (1)
  {
    ...
    if (unit->shared.previtem != 0xffff)
    {
      ...
      {
        u32 unitnum = ((u8*)unit - (u8*)polygon->poly->unit[0]) / polygon->poly->unit_size;
        u32 new_count_next;
        ...
        } while (!prevunit->shared.count_next.compare_exchange_weak(
          orig_count_next, new_count_next, std::memory_order_release, std::memory_order_relaxed));
    ...
    } while (!unit->shared.count_next.compare_exchange_weak(orig_count_next, 0, std::memory_order_release,
