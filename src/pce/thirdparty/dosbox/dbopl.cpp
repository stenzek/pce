...
Bit32s Operator::RateForward(Bit32u add)
{
  rateIndex += add;
...
Bitu Operator::ForwardVolume()
{
  return currentLevel + (this->*volHandler)();
}

Bitu Operator::ForwardWave()
{
...
void Operator::SetState(Bit8u s)
{
  state = s;
  volHandler = VolumeHandlerTable[s];
}

bool Operator::Silent() const
{
...
void Operator::Prepare(const Chip* chip)
{
...
Bits Operator::GetWave(Bitu index, Bitu vol)
{
#if (DBOPL_WAVE == WAVE_HANDLER)
...
Bits Operator::GetSample(Bits modulation)
{
