#include "pce/systems/pcUM498F.h"
...
#include "pce/bus.h"
#include "pce/cpu.h"
Log_SetChannel(Systems::PCUM498F);

namespace Systems {

PCUM498F::PCUM498F(HostInterface* host_interface, CPU_X86::Model model /* = CPU_X86::MODEL_486 */,
                       float cpu_frequency /* = 8000000.0f */, uint32 memory_size /* = 16 * 1024 * 1024 */)
  : PCBase(host_interface), m_bios_file_path("romimages/um498f.awa")
{
...
void PCUM498F::Reset()
{
  ...
  std::fill(m_UM498F_registers.begin(), m_UM498F_registers.end(), uint8(0));
  m_UM498F_index_register = 0;
  m_UM498F_unlock_stage = 2;
  ...
  // PIT #2 gate OFF, speaker OFF.
  m_timer->SetChannelGateInput(2, false);
  m_speaker->SetOutputEnabled(false);
}

bool PCUM498F::LoadSystemState(BinaryReader& reader)
{
...
bool PCUM498F::SaveSystemState(BinaryWriter& writer)
{
...
void PCUM498F::ConnectSystemIOPorts()
{
  m_bus->ConnectIOPortRead(0x0028, this, std::bind(&PCUM498F::IOReadUM498FIndexRegister, this, std::placeholders::_2));
  m_bus->ConnectIOPortWrite(0x0028, this, std::bind(&PCUM498F::IOWriteUM498FIndexRegister, this, std::placeholders::_2));
  m_bus->ConnectIOPortRead(0x002A, this, std::bind(&PCUM498F::IOReadUM498FDataRegister, this, std::placeholders::_2));
  m_bus->ConnectIOPortWrite(0x002A, this, std::bind(&PCUM498F::IOWriteUM498FDataRegister, this, std::placeholders::_2));
  // System control ports
  m_bus->ConnectIOPortRead(0x0092, this, std::bind(&PCUM498F::IOReadSystemControlPortA, this, std::placeholders::_2));
  m_bus->ConnectIOPortWrite(0x0092, this,
                            std::bind(&PCUM498F::IOWriteSystemControlPortA, this, std::placeholders::_2));
  m_bus->ConnectIOPortRead(0x0061, this, std::bind(&PCUM498F::IOReadSystemControlPortB, this, std::placeholders::_2));
  m_bus->ConnectIOPortWrite(0x0061, this,
                            std::bind(&PCUM498F::IOWriteSystemControlPortB, this, std::placeholders::_2));
  ...
  // Eat these since they're very noisy during startup..
  m_bus->ConnectIOPortWrite(0x00E1, this, [](uint32 port, uint8 value) {});
  m_bus->ConnectIOPortWrite(0x00E2, this, [](uint32 port, uint8 value) {});
}

void PCUM498F::IOReadUM498FIndexRegister(uint8* value)
{
  if (m_UM498F_unlock_stage != 0)
  {
    *value = 0xFF;
    return;
  }

  *value = m_UM498F_index_register;
}

void PCUM498F::IOWriteUM498FIndexRegister(uint8 value)
{
  if (m_UM498F_unlock_stage == 0)
  {
    m_UM498F_index_register = value;
    return;
  }
  else if (m_UM498F_unlock_stage == 1)
  {
    if (value == 0x05)
      m_UM498F_unlock_stage = 0;
    else
      m_UM498F_unlock_stage = 2;
  }
  else
  {
    if (value == 0xA0)
      m_UM498F_unlock_stage = 1;
    else
      m_UM498F_unlock_stage = 2;
  }
}

void PCUM498F::IOReadUM498FDataRegister(uint8* value)
{
  if (m_UM498F_unlock_stage != 0)
  {
    *value = 0xFF;
  ...
  Log_WarningPrintf("UM498F Register Read 0x%02X -> 0x%02X", m_UM498F_index_register, m_UM498F_registers[m_UM498F_index_register]);
  *value = m_UM498F_registers[m_UM498F_index_register];
}

void PCUM498F::IOWriteUM498FDataRegister(uint8 value)
{
  if (m_UM498F_unlock_stage != 0)
    return;

  Log_WarningPrintf("UM498F Register Write 0x%02X <- 0x%02X", m_UM498F_index_register, value);
  m_UM498F_registers[m_UM498F_index_register] = value;

//   if (m_UM498F_index_register == 0x13 || m_UM498F_index_register == 0x14)
//   {
...
void PCUM498F::UpdateShadowRAM()
{
  for (uint32 i = 0; i < 8; i++)
  {
    uint32 base = UINT32_C(0xC0000) + (i << 15);
    if (m_UM498F_registers[0x13] & (1 << i))
    {
      // Shadowing enabled for this region.
      const uint32 flag = m_UM498F_registers[0x14] & 0x03;
...
void PCUM498F::IOReadSystemControlPortA(uint8* value)
{
  *value = (BoolToUInt8(m_cmos_lock) << 3) | (BoolToUInt8(GetA20State()) << 1);
}

void PCUM498F::IOWriteSystemControlPortA(uint8 value)
{
...
void PCUM498F::IOReadSystemControlPortB(uint8* value)
{
...
void PCUM498F::IOWriteSystemControlPortB(uint8 value)
{
...
void PCUM498F::UpdateKeyboardControllerOutputPort()
{
...
void PCUM498F::SetCMOSVariables()
{
