...
private:
  ...
  static void FallbackToInterpreter(CPU* cpu)
  {
    ...
    Callback callback(cpu);
    if (!DecodeInstruction(&instruction, cpu->m_current_address_size, cpu->m_current_operand_size, callback))
    {
    ...
    Interpreter::ExecuteInstruction(cpu, &instruction);
  }
