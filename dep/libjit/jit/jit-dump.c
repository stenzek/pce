...
void jit_dump_type(FILE* stream, jit_type_t type)
{
  ...
  switch (type->kind)
  {
    ...
    {
      fprintf(stream, "struct<%u>", (unsigned int)(jit_type_get_size(type)));
      return;
    }
      /* Not reached */
    ...
    case JIT_TYPE_UNION:
    {
      fprintf(stream, "union<%u>", (unsigned int)(jit_type_get_size(type)));
      return;
    }
      /* Not reached */
...
/*@
 * @deftypefun void jit_dump_value (FILE *@var{stream}, jit_function_t @var{func}, jit_value_t @var{value}, const char
*@var{prefix})
...
void jit_dump_value(FILE* stream, jit_function_t func, jit_value_t value, const char* prefix)
{
  ...
  if (value->is_constant)
  {
    ...
    switch ((jit_type_promote_int(jit_type_normalize(const_value.type)))->kind)
    {
      case JIT_TYPE_INT:
      {
        if (const_value.un.int_value < 0)
        {
          name = format_integer(buf, 1, (jit_ulong)(jit_uint)(-(const_value.un.int_value)));
        }
        else
        {
          name = format_integer(buf, 0, (jit_ulong)(jit_uint)(const_value.un.int_value));
        }
      }
      break;
      ...
      case JIT_TYPE_UINT:
      {
        name = format_integer(buf, 0, (jit_ulong)(const_value.un.uint_value));
      }
      break;
      ...
      case JIT_TYPE_LONG:
      {
        ...
        {
          name = format_integer(buf, 1, (jit_ulong)(-(const_value.un.long_value)));
        }
        else
        {
          name = format_integer(buf, 0, (jit_ulong)(const_value.un.long_value));
        }
      }
      break;
      ...
      case JIT_TYPE_FLOAT32:
      {
        jit_snprintf(buf, sizeof(buf), "%f", (double)(const_value.un.float32_value));
        name = buf;
      }
      break;
      ...
      case JIT_TYPE_FLOAT64:
      {
        jit_snprintf(buf, sizeof(buf), "%f", (double)(const_value.un.float64_value));
        name = buf;
      }
      break;
      ...
      case JIT_TYPE_NFLOAT:
      {
        jit_snprintf(buf, sizeof(buf), "%f", (double)(const_value.un.nfloat_value));
        name = buf;
      }
      break;
  ...
  block_size = func->builder->value_pool.elem_size * func->builder->value_pool.elems_per_block;
  posn = 1;
  while (block != 0)
  {
    if (((char*)value) >= block->data && ((char*)value) < (block->data + block_size))
    {
      posn += (((char*)value) - block->data) / func->builder->value_pool.elem_size;
      break;
    }
 ...
 */
static void dump_value(FILE* stream, jit_function_t func, jit_value_t value, int type)
{
...
void jit_dump_insn(FILE* stream, jit_function_t func, jit_insn_t insn)
{
  ...
  {
    if (insn->value1)
      fprintf(stream, "%s %s (0x%08lx)", name, (const char*)(insn->value1), (long)(jit_nuint)(insn->dest));
    else
      fprintf(stream, "%s 0x08%lx", name, (long)(jit_nuint)(insn->dest));
    return;
  }
  ...
  else if ((flags & JIT_OPCODE_IS_ADDROF_LABEL) != 0)
  {
    ...
    fprintf(stream, " = ");
    fprintf(stream, "address_of_label .L%ld", (long)(jit_insn_get_label(insn)));
    return;
  }
  ...
  if ((flags & JIT_OPCODE_DEST_MASK) != JIT_OPCODE_DEST_EMPTY && !jit_insn_dest_is_value(insn))
  {
    dump_value(stream, func, jit_insn_get_dest(insn), flags & JIT_OPCODE_DEST_MASK);
    fprintf(stream, " = ");
  }
  ...
  switch (flags & JIT_OPCODE_OPER_MASK)
  {
    case JIT_OPCODE_OPER_ADD:
      infix_name = " + ";
      break;
    case JIT_OPCODE_OPER_SUB:
      infix_name = " - ";
      break;
    case JIT_OPCODE_OPER_MUL:
      infix_name = " * ";
      break;
    case JIT_OPCODE_OPER_DIV:
      infix_name = " / ";
      break;
    case JIT_OPCODE_OPER_REM:
      infix_name = " % ";
      break;
    case JIT_OPCODE_OPER_NEG:
      infix_name = "-";
      break;
    case JIT_OPCODE_OPER_AND:
      infix_name = " & ";
      break;
    case JIT_OPCODE_OPER_OR:
      infix_name = " | ";
      break;
    case JIT_OPCODE_OPER_XOR:
      infix_name = " ^ ";
      break;
    case JIT_OPCODE_OPER_NOT:
      infix_name = "~";
      break;
    case JIT_OPCODE_OPER_EQ:
      infix_name = " == ";
      break;
    case JIT_OPCODE_OPER_NE:
      infix_name = " != ";
      break;
    case JIT_OPCODE_OPER_LT:
      infix_name = " < ";
      break;
    case JIT_OPCODE_OPER_LE:
      infix_name = " <= ";
      break;
    case JIT_OPCODE_OPER_GT:
      infix_name = " > ";
      break;
    case JIT_OPCODE_OPER_GE:
      infix_name = " >= ";
      break;
    case JIT_OPCODE_OPER_SHL:
      infix_name = " << ";
      break;
    case JIT_OPCODE_OPER_SHR:
      infix_name = " >> ";
      break;
    case JIT_OPCODE_OPER_SHR_UN:
      infix_name = " >>> ";
      break;
    case JIT_OPCODE_OPER_COPY:
      infix_name = "";
      break;
    case JIT_OPCODE_OPER_ADDRESS_OF:
      infix_name = "&";
      break;
  }
  if (infix_name)
  {
    if ((flags & JIT_OPCODE_SRC2_MASK) != 0)
    {
      ...
      dump_value(stream, func, jit_insn_get_value1(insn), flags & JIT_OPCODE_SRC1_MASK);
      fputs(infix_name, stream);
      dump_value(stream, func, jit_insn_get_value2(insn), flags & JIT_OPCODE_SRC2_MASK);
    }
    else
    {
...
static void dump_interp_code(FILE* stream, void** pc, void** end)
{
  ...
  while (pc < end)
  {
    ...
    switch (info->flags & JIT_OPCODE_INTERP_ARGS_MASK)
    {
      ...
      {
        fprintf(stream, " %ld, %ld", (long)(jit_nint)(pc[0]), (long)(jit_nint)(pc[1]));
        pc += 2;
      }
      break;
      ...
      case JIT_OPCODE_CONST_LONG:
      {
        ...
        pc += (sizeof(jit_ulong) + sizeof(void*) - 1) / sizeof(void*);
        fprintf(stream, " 0x%lX%08lX", (long)((value >> 32) & jit_max_uint), (long)(value & jit_max_uint));
      }
      break;
      ...
      case JIT_OPCODE_CONST_FLOAT32:
      {
        ...
        pc += (sizeof(jit_float32) + sizeof(void*) - 1) / sizeof(void*);
      ...
      case JIT_OPCODE_CONST_FLOAT64:
      {
        ...
        pc += (sizeof(jit_float64) + sizeof(void*) - 1) / sizeof(void*);
        fprintf(stream, " %f", (double)value);
      }
      break;
      ...
      case JIT_OPCODE_CONST_NFLOAT:
      {
        ...
        pc += (sizeof(jit_nfloat) + sizeof(void*) - 1) / sizeof(void*);
        fprintf(stream, " %f", (double)value);
      }
      break;
      ...
      default:
      {
        if ((info->flags & (JIT_OPCODE_IS_BRANCH | JIT_OPCODE_IS_ADDROF_LABEL)) != 0)
        {
          fprintf(stream, " %08lX", (long)(jit_nint)((pc - 1) + (jit_nint)(*pc)));
          ++pc;
        }
        ...
        {
          fprintf(stream, " 0x%lX, %ld", (long)(jit_nint)(pc[1]), (long)(jit_nint)(pc[2]));
          pc += 3;
        }
        else if ((info->flags & JIT_OPCODE_IS_JUMP_TABLE) != 0)
        {
          ...
          {
            fprintf(stream, " %lX", (long)(jit_nint)pc[label]);
          }
          pc += 1 + num_labels;
        }
      }
...
static void dump_object_code(FILE* stream, void* start, void* end)
{
  ...
  system(cmdline);
  sprintf(cmdline, "objdump --adjust-vma=%ld -d %s > %s", (long)(jit_nint)start, o_path, s_path);
  system(cmdline);
  file = fopen(s_path, "r");
  if (file)
  {
...
void jit_dump_function(FILE* stream, jit_function_t func, const char* name)
{
  ...
  else if (func->is_compiled)
  {
    ...
    fprintf(stream, "\t%08lX: prolog(0x%lX, %d, %d, %d)\n", (long)(jit_nint)interp, (long)(jit_nint)func,
            (int)(interp->args_size), (int)(interp->frame_size), (int)(interp->working_area));
